...
- **2026-06-02** — Sessão 4 (auditoria v3.0): cross_squad → **12 squads** (+cybersecurity, movement, pre-programming, storytelling); `docs/handoff-matrix.md` (handoffs intra+cross explícitos); relatório reescrito p/ estrutura v3 (scores 0-100 por seção/capacidade + severidade). **97/100 SOTA**.
- **2026-06-02** — Sessão 5 (Top-5 upgrades): **#1** hook pre-commit (`scripts/hooks/pre-commit` + `install-hooks.sh`) regenera matrizes + qa-strict/crosslink/id/citation; **#2** `readiness-check --scorecard` (scorecard vivo); **#3** handoff executável deepresearch→offerbook + `handoff-validate.py`; **#4** `kpi-snapshot.py` + snapshot exemplo (20 KPIs); **#5** backlog Kaizen vivo + `backlog-prioritize.py` (ROI numérico ranqueado). Hook ativado. qa --strict **100/100**, 0 quebrados, bijeção ok, 0 citações.
